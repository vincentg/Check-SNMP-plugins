...
void init_v3_args(snmpv3_args_t *v3_args) { memset(v3_args, 0, sizeof(snmpv3_args_t)); }
...
void snmpv3_set_session(netsnmp_session *session, const snmpv3_args_t *v3args)
{
    ...
    if (session->securityLevel == SNMP_SEC_LEVEL_AUTHPRIV) {
      ...
      session->securityPrivKeyLen = USM_PRIV_KU_LEN;
		  kuret = generate_Ku(session->securityAuthProto, session->securityAuthProtoLen, (unsigned char *)v3args->priv_password,
							strlen(v3args->priv_password), session->securityPrivKey, &(session->securityPrivKeyLen));
