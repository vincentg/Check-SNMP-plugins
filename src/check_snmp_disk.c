			...
			for (vars = response->variables; vars; vars = vars->next_variable)
			{
				...
				if ((int)vars->name[10] == 2)
				{
					...
					if (vars->type == ASN_OBJECT_ID)
					{
						...
						else if ((check_vmem == 1) &&
								 (!memcmp((vars->val).objid, VIRTUAL_MEM, typelen)))
						{
							virtual_id = (int)vars->name[11];
						}
						else if ((check_net == 1) && (!memcmp((vars->val).objid,
...
int check_and_print(t_storage *storage, int storage_length)
{
	...
	if (perfdata)
	{
		...
		{
			printf("'disk%d_label'=%s,'disk%d_used\'=%.0fKB,\'disk%d_total\'=%.0fKB,\'disk%d_percent\'=%.2f%%;%d;%d", count, current_storage->descr, count, current_storage->allocunit * (double)current_storage->used / 1024, count, current_storage->allocunit * (double)current_storage->totalsize / 1024, count, ((double)current_storage->used / current_storage->totalsize) * 100, warningmin, criticalmin);
